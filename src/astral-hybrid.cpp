...
struct Workflow {
	...
	void readInputTrees(string line, string mapping) {
		if (mapping != ""){
	...
	void operator()(string line, string constraintTree) {
		int dupType = 1;
		string mappingFile;
		meta.initialize();
		LOG.enabled = false;
		meta.constraintFile = "";
		meta.constraintTree = constraintTree;
		...
		readInputTrees(line, mappingFile);
...
int main(int argc, char** argv){
	...
	MetaAlgorithm::staticInitialize(argc, argv);
	string constraintTree;

	if (ARG.getStringArg("constraint") != "") {
		ifstream fin(ARG.getStringArg("constraint"));
